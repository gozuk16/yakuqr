...
JAHISの院外処方箋２次元シンボル記録条件規約（Ver.1.0〜Ver.2.6）に準拠したQRコードを読み取るCLIツール。画像ファイル（PNG/JPEG）またはPDFファイルを入力として受け取り、QRコードの内容を解析してテキストファイルに出力する。また、JAHIS仕様との適合チェックを行い、不適合項目を指摘する。

## 対応バージョン

| JAHIS仕様書 | データフォーマット | JAHISCTヘッダー | レコード構造 |
|---|---|---|---|
| Ver.1.0 (2012/9) | 1 | JAHISTC01 | レコード2=患者, レコード6=薬品 |
| Ver.1.1 (2013/9) | 2 | JAHISTC02 | レコード2=患者, レコード6=薬品 |
| Ver.2.0 (2015/11) | 3 | JAHISTC03 | レコード2=患者, レコード6=薬品 |
| Ver.2.1 (2016/3) | 4 | JAHISTC04 | レコード1=患者, レコード201=薬品 |
| Ver.2.2 (2017/11) | 5 | JAHISTC05 | レコード1=患者, レコード201=薬品 |
| Ver.2.3 (2019/4) | 6 | JAHISTC06 | レコード1=患者, レコード201=薬品 |
| Ver.2.4 (2020/3) | 7 | JAHISTC07 | レコード1=患者, レコード201=薬品 |
| Ver.2.5 (2024/3) | 8 | JAHISTC08 | レコード1=患者, レコード201=薬品 |
| Ver.2.6 (2024/9) | 9 | JAHISTC09 | レコード1=患者, レコード201=薬品 |

**注意:** 旧バージョン（JAHISTC01〜03）は1桁形式（JAHISTC1等）でも記録されている場合がある。バージョン未検出時はエラーとして処理し、フォールバックしない。
...
- ヘッダレコードからバージョン（Ver.1.0〜Ver.2.6）を自動検出。検出不能な場合はエラーとして処理し、フォールバックしない
