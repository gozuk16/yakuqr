...
- JAHISの分割符号（先頭レコードが `9,N,M` 形式 — N番目/全M枚）を検出し、連番順に結合
- 分割QRが揃っていない場合（枚数不足）は WARNING を出力し、取得済み分で処理を続行
- ヘッダレコードからバージョン（Ver.2〜Ver.4）を自動検出。検出不能な場合は最新版として扱い INFO を出力
